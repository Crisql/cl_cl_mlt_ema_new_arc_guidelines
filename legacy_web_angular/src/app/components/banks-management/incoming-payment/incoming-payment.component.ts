...
import {CheckboxColumnSelection} from "../../../interfaces/i-table-data";
import {
  ACCOUNT_TYPE,
  IAccount,
  ICurrency,
...
import {IIncomingPayment, IPaymentCreditCards, IPaymentInvoices} from "../../../interfaces/i-incoming-payment";
...
} from "@Component/banks-management/incoming-payment/reconciliation/reconciliation.component";
import {
...
export class IncomingPaymentComponent implements OnInit, OnDestroy {
  ...
  editableField: IEditableField<IPermissionbyUser>[] = [
    {
      ColumnName: 'Pago',
      Permission: {Name: 'Banks_IncomingPayment_EditPayment'}
    },
  ];
  listInInvoices: IPayInAccount[] = [];
  /*Formularios*/
  ...
  public Clear(): void {
    ...
    forkJoin({
      ...
      next:(callback)=> {
        ...
        this.terminals = callback.Terminal.Data;
  ...
  public GetTotalByCurrency(_pay: number, _currency: string): number {
  ...
  HandlePaymentModalResult = (_event: ICLEvent): void => {
    ...
    if (data.Result) {
      ...
      this.CreateIncomingPayment(data.PaymentState, this.currentSession!.Rate);
    } else {
  ...
  private GetDocuments = (): void => {
    ...
    this.documentService.GetDocumentForPayment(this.DefaultBusinessPartner?.CardCode ?? '', data.DocCurrency === '##' ? "" : data.DocCurrency, data.DateFrom, data.DateTo).pipe(
      finalize(() => this.overlayService.Drop())
    ).subscribe({
      next: (callback) => {
        this.documents = (callback.Data || []).map(document => {
          return {
            ...document,
            Pago: 0,
            SaldoTable: document.DocCurrency === this.localCurrency.Id ? CLTofixed(this.DecimalTotalDocument, document.Saldo) : CLTofixed(this.DecimalTotalDocument, document.SaldoUSD),
            TotalTable: document.DocCurrency === this.localCurrency.Id ? CLTofixed(this.DecimalTotalDocument, document.Total) : CLTofixed(this.DecimalTotalDocument, document.TotalUSD),
  ...
  public OpenPaymentModal(): void {
    if (this.documentForm.controls['IsPayAccount'].value && +(this.documentForm.controls['Amount'].value) <= 0) {
      this.alertsService.Toast({
    ...
    if (!this.documentForm.controls['IsPayAccount'].value && !this.documents.some(x => x.Assigned)) {
      this.alertsService.Toast({
        type: CLToastType.INFO,
  ...
  /**
   * Method to load Reconciliation modal
   * @constructor
   */
  public OpentReconciliationModal(): void {

    let currency = this.currencies.find(x => x.Id === this.documentForm.get('DocCurrency')?.value);

    let data = {
      Customer: this.DefaultBusinessPartner,
      Currency: currency?.Id,
      listInvoice: this.listInInvoices,
      DecimalDocument: this.DecimalTotalDocument,
      ExchangeRate: this.currentSession.Rate,
      Currencies: this.currencies
    } as IDataForReconciliationModal;


    this.matDialog
      .open(ReconciliationComponent, {
      ...
      .afterClosed()
      .pipe(
        switchMap((result: InternalReconciliationsResponse) => {
          if (result) {
            this.documents = this.documents.filter(x => !x.Assigned);
            this.total = this.documents
              .filter(x => x.Assigned)
              .reduce((acc, value) => acc + value.Pago, 0);
            this.totalFC = this.total / this.currentSession.Rate;
            this.InflateTable();
            return this.modalService.Continue({
              title: `La reconciliación se creó correctamente con el número de documento ${result.ReconNum}`,
  ...
  public SaveChanges(): void {
    ...
    this.LoadAccounts().pipe(
      ...
      next: (callback) => {
        ...
        let isAllCurrency: boolean = currency === '##';

        currency = isAllCurrency ? this.currencies.find(x => x.IsLocal)?.Id || '' : currency;
        ...
        this.totalFC = CLTofixed(this.DecimalTotalDocument, this.totalFC);
        ...
        this.paymentHolder.PaymentSettings.CanEditCardNumber = this.permissions.some(x => x.Name === 'Banks_IncomingPayment_EditPayCardNumber');
        ...
        this.paymentHolder.PaymentSettings.CardRefundAmount = 0;
        this.paymentHolder.PaymentSettings.DocTotal = (this.documentForm.controls['IsPayAccount'].value) ? this.documentForm.controls['Amount'].value : currency === this.localCurrency.Id ? this.total : this.totalFC;
  ...
  /**
   * Method to create payment received
   * @param _paymentState - Object recived of payment modal
   * @param _rate - Company exchange rate
   * @constructor
   */
  public CreateIncomingPayment(_paymentState: IPaymentState, _rate: number): void {
    ...
    let incomingPayment = {
      ...
      PaymentInvoices: this.documents.filter(_x => _x.Assigned).map(document => {
        return {
          InvoiceType: this.InvoiceType(document.DocumentType),
          DocEntry: document.DocEntry,
          SumApplied: document.DocCurrency === this.localCurrency.Id ? document.Pago : CLTofixed(this.DecimalTotalDocument, (document.Pago * _rate)),
          AppliedFC: document.DocCurrency !== this.localCurrency.Id ? document.Pago : CLTofixed(this.DecimalTotalDocument, (document.Pago / _rate))
        } as IPaymentInvoices
      }),
    ...
    this.overlayService.OnPost();
    this.incomingPaymentService.Post(incomingPayment).pipe(
      switchMap(res => {
        this.overlayService.Drop();
        let detailIncomingPayment = {
          DocEntry: res.Data.DocEntry,
          DocNum: res.Data.DocNum,
          NumFE: '',
          CashChange: _paymentState.Currency === this.localCurrency.Id ? _paymentState.ChangeAmount : this.DisplayChangeAmount(this.localCurrency.Id, _paymentState.ChangeAmount),
          ...
          Title: 'Pago recibido',
          Accion: 'creado',
          TypeReport: 'ReceivedPayment'
        } as ISuccessSalesInfo;

        return this.OpenDialogSuccessSales(detailIncomingPayment);
      ...
      error: (err) => {
        this.modalService.Continue({
          title: 'Se produjo un error creando el pago recibido',
  ...
  private InvoiceType(_typeDocument: string): string {
    switch (_typeDocument) {
      case 'Anticipo':
        return PaymentInvoiceType.it_DownPayment
        break;
      default:
        return PaymentInvoiceType.it_Invoice
    }
  }
...
//#region RECONCILIACION
  ...
  public Reconciliation(): void {
    ...
    if (this.documents.some(x => x.Assigned)) {
      ...
      this.listInInvoices = this.documents.filter(x => x.Assigned).map(element => {
        return {
          DocEntry: element.DocEntry,
          DocNum: element.DocNum,
          DocTotal: element.TotalTable,
          DocCurrency: element.DocCurrency,
          ReconcileAmount: element.Pago,
          TransId: element.TransId,
          ObjType: element.ObjType,
          Saldo: element.SaldoTable,
          DocumentType: element.DocumentType,
          DocDate: element.DocDate
        } as IPayInAccount
      });
      this.OpentReconciliationModal();
    } else {
      this.alertsService.Toast({
        type: CLToastType.INFO,
        message: `No ha seleccionado documentos`
  ...
  public SetInitialData(): void {
    ...
    if (this.settings) {
      ...
      if (companyDecimal && companyDecimal.length > 0) {
        ...
        if (decimalCompany) {
          this.TO_FIXED_TOTALDOCUMENT = `1.${decimalCompany?.TotalDocument || 0}-${decimalCompany?.TotalDocument || 0}`;
