...
export class ApprovalsComponent implements OnInit, OnDestroy, AfterViewInit{
      ...
      this.activatedRoute.data.subscribe({
        next: (data) => {
          ...
          if (resolvedData) {
            ...
            this.filteredSalesPersons$ = of(this.salesPersons.slice(0, 10));
      ...
      case CL_ACTIONS.CONTINUE:
        ...
        (
          ...
          switchMap(res=>{
            ...
            if(payTermType == Payterm.Counted  && this.draft?.Approval_Status == 'arsApproved' && (approvalRequest.ObjectType!=DocumentTypeCode.SalesOrders.toString() && approvalRequest.ObjectType!=DocumentTypeCode.SalesQuotations.toString())){
                ...
                .pipe(finalize(() => {
                  ...
                  const NEW_TABLE_STATE = {
                    Records: this.documents.map(d => this.sharedService.MapTableColumns({
                      ...
                      DocDateFormatted: formatDate(`${d.CreationDate}`.replace('00:00:00', d.CreationTime), 'MMMM d, y hh:mm a', 'en'),
                      DecisionRemarks: ''
